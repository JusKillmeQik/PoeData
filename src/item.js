...
const PoeData = require('./poe-data.js');
  ...
  */
  ...
  analyseRequirements(lines) {
    ...
    for (let l = 0; l < lines.length; l++) {
      ...
      if (match) {
        let name = match[1];
        this.requirements[name] = (typeof match[3] !== "undefined" ? match[3] : true);
      } else {
  ...
  analyseProperties(lines) {
    let propertyPrefix = "";
    if (this.hasTag("gem")) {
      if (!this.properties.hasOwnProperty("Gem Tags")) {
  ...
  detectBlockType(lines) {
    ...
    for (let typeIndicator in parserBlockTypes) {
      for (let l = 0; l < lines.length; l++) {
        let regex = parserBlockTypes[typeIndicator].regex;
        if(lines[l].includes(regex)) {
  ...
  getMaxRolledMods() {
    let maxRolledMods = [];
    for (let i = 0; i < this.modsImplicit.length; i++) {
      if (this.modsImplicit[i].isMaxRoll()) {
        maxRolledMods.push(this.modsImplicit[i]);
      }
    }
    for (let i = 0; i < this.modsExplicit.length; i++) {
      if (this.modsExplicit[i].isMaxRoll()) {
        maxRolledMods.push(this.modsExplicit[i]);
      }
    }
    return maxRolledMods;
  }
  getImplicitMods() {
    ...
    if (this.modsImplicitPossible === null) {
      ...
      if (this.itemBase !== null) {
        ...
        for (let i = 0; i < this.itemBase['mods'].length; i++) {
          ...
          if (parseInt(itemMod['is implicit']) !== 1) {
            continue;
          }
  ...
  isFourSocket() {
    return (this.hasTag("gloves") || this.hasTag("boots") || this.hasTag("helmet"));
  }
  isThreeSocket() {
    return (this.hasTag("one_hand_weapon") || this.hasTag("shield") || this.hasTag("quiver"));
  }
  isSingleSocket() {
    return this.hasTag("ring");
  }
  isCorrupted() {
